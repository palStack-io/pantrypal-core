...
## brainPal — keep the map in sync

The ecosystem map at `brainPal/` (monorepo root) is generated by scanning this
repo — compose files, deploy stacks, module registries, and the root `docs/` folder.

After any **major change** here (new module, service, integration, AI endpoint,
port, tier, pricing, or rename), keep the map accurate:

1. Update **docs/pantrypal.md** at the monorepo root — the scanner reads root docs for
   cross-references and renders them in the map.
2. Re-run the scanner: `node brainPal/scanner/scan.js`
3. New cross-product relationships, external services, or planned features
   belong in `brainPal/graph/overlay.json` (hand-curated, scanner never edits it).
