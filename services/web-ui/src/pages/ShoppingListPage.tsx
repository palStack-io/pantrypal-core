...
export function ShoppingListPage() {
  ...
  const { items, loading, addItem, deleteItem, toggleChecked, clearChecked, importCheckedToInventory, suggestLowStock } = useShoppingItems();
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
      await addItem({ name: newItemName.trim(), quantity: newItemQuantity, category: 'Uncategorized' });
      setNewItemName('');
  ...
  const handleDeleteShoppingItem = (itemId: number | string) => {
    const undo = deleteItem(itemId);
    toast.show({ message: 'Item removed.', type: 'info', duration: 5500, action: { label: 'Undo', onClick: undo } });
  };

  const handleClearChecked = async () => {
    const checkedCount = items.filter(i => i.checked).length;
    if (!checkedCount) return;
    if (!await dialog.confirm('All checked items will be removed from the list.', { title: 'Clear Checked Items', confirmLabel: 'Clear', icon: '🛒' })) return;
    const undo = clearChecked();
    toast.show({ message: `${checkedCount} item${checkedCount !== 1 ? 's' : ''} cleared.`, type: 'info', duration: 5500, action: { label: 'Undo', onClick: undo } });
  };
    ...
    <div style={{ padding: '32px', backgroundColor: colors.background, minHeight: '100vh' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ fontSize: '32px', fontWeight: '900', color: colors.textPrimary, margin: 0, display: 'flex', alignItems: 'center', gap: '12px' }}>
              <ShoppingCart size={36} /> Shopping List
            </h1>
            <p style={{ color: colors.textSecondary, marginTop: '8px' }}>{stats.remaining} items remaining • {stats.checked} checked</p>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
          <div style={{ background: colors.card, borderRadius: borderRadius.xl, padding: '32px', border: `1px solid ${colors.border}` }}>
                ...
                <div key={category} style={{ marginBottom: groupBy === 'category' ? '24px' : '0' }}>
                  {groupBy === 'category' && (
                    <div style={{ fontSize: '18px', fontWeight: '800', color: colors.textPrimary, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {category}<span style={{ color: colors.textSecondary, fontSize: '14px' }}>({categoryItems.length})</span>
                    </div>
                  )}
                  {categoryItems.map(item => (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', background: colors.background, borderRadius: borderRadius.lg, marginBottom: '12px', opacity: item.checked ? 0.5 : 1, transition: 'all 0.2s' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {item.quantity > 1 && <div style={{ fontSize: '14px', color: colors.textSecondary }}>Qty: {item.quantity}</div>}
                      </div>
                      <button onClick={() => handleDeleteShoppingItem(item.id)} style={{ width: '32px', height: '32px', borderRadius: '6px', background: 'white', border: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: 0.6, transition: 'all 0.2s' }}
                        onMouseEnter={(e) => { e.currentTarget.style.background = '#fef2f2'; e.currentTarget.style.borderColor = '#fca5a5'; e.currentTarget.style.opacity = '1'; }}
                        onMouseLeave={(e) => { e.currentTarget.style.background = 'white'; e.currentTarget.style.borderColor = colors.border; e.currentTarget.style.opacity = '0.6'; }}>
            ...
            {stats.checked > 0 && (
              <div style={{ paddingTop: '24px', borderTop: `1px solid ${colors.border}`, marginTop: '24px' }}>
                <button onClick={handleImportChecked} style={{ width: '100%', padding: '16px', borderRadius: borderRadius.lg, fontSize: '15px', fontWeight: '700', background: 'linear-gradient(135deg, #f59e0b, #fbbf24)', color: 'white', border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(245, 158, 11, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          ...
          <div>
            <div style={{ background: colors.card, borderRadius: borderRadius.xl, padding: '24px', border: `1px solid ${colors.border}`, marginBottom: '24px' }}>
              <h3 style={{ fontSize: '18px', fontWeight: '800', color: colors.textPrimary, marginBottom: '16px' }}>Summary</h3>
            ...
            <div style={{ background: colors.card, borderRadius: borderRadius.xl, padding: '24px', border: `1px solid ${colors.border}` }}>
              <h3 style={{ fontSize: '18px', fontWeight: '800', color: colors.textPrimary, marginBottom: '16px' }}>Quick Actions</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <button onClick={handleSuggestLowStock} style={{ padding: '14px', background: colors.background, borderRadius: borderRadius.lg, fontSize: '14px', fontWeight: '600', color: colors.textPrimary, cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '10px', border: `1px solid ${colors.border}` }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = '#fffbeb'; e.currentTarget.style.color = '#f59e0b'; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = colors.background; e.currentTarget.style.color = colors.textPrimary; }}>
                  <Package size={18} /> Add Low Stock Items
                </button>
                <button onClick={handleClearChecked} disabled={stats.checked === 0} style={{ padding: '14px', background: colors.background, borderRadius: borderRadius.lg, fontSize: '14px', fontWeight: '600', color: stats.checked > 0 ? colors.textPrimary : colors.textSecondary, cursor: stats.checked > 0 ? 'pointer' : 'not-allowed', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '10px', border: `1px solid ${colors.border}`, opacity: stats.checked > 0 ? 1 : 0.5 }}
                  onMouseEnter={(e) => { if (stats.checked > 0) { e.currentTarget.style.background = '#fef2f2'; e.currentTarget.style.color = '#dc2626'; } }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = colors.background; e.currentTarget.style.color = stats.checked > 0 ? colors.textPrimary : colors.textSecondary; }}>
                  <Trash2 size={18} /> Clear Checked Items
                </button>
              </div>
            </div>
          </div>
