...
export function Sidebar({ isOpen, currentPath, onNavigate, onFilterChange, currentFilters = {} }: SidebarProps) {
  ...
  useEffect(() => {
    if (items.length > 0) {
      ...
      const locs: Record<string, number> = {}, cats: Record<string, number> = {};
      items.forEach(item => {
  ...
  return (
    ...
    >
      ...
      {count !== undefined && count > 0 && (
        <span style={{ background: active ? 'rgba(255, 255, 255, 0.3)' : colors.accentBg, padding: '2px 8px', borderRadius: '12px', fontSize: '12px', fontWeight: '600' }}>
          {count}
        </span>
      )}
    </div>
