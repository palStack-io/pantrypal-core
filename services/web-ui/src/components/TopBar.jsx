// FloatingActions — replaces the old top bar
  ...
  return (
    <div style={{
      ...
      <button
        ...
        onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.12)'; e.currentTarget.style.transform = 'none'; }}
      >
      ...
      <div ref={notificationRef} style={{ position: 'relative' }}>
        ...
        {showNotifications && (
          <div style={{
            ...
            {expiringItems.length === 0 ? (
              <div style={{ padding: `${spacing.xl} ${spacing.lg}`, textAlign: 'center', color: colors.textSecondary, fontSize: '14px' }}>
                No expiring items
              </div>
            ) : (
              expiringItems.map(item => {
                ...
                return (
                  <div key={item.id} style={{ padding: `${spacing.md} ${spacing.lg}`, borderBottom: `1px solid ${colors.border}`, transition: 'background 0.15s', cursor: 'default' }}
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: spacing.sm }}>
                      ...
                      <span style={{ padding: `${spacing.xs} ${spacing.sm}`, backgroundColor: statusColor + '20', color: statusColor, borderRadius: borderRadius.sm, fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                        {status}
                      </span>
                    </div>
                  </div>
          ...
          {showUserMenu && (
            <div style={{
              ...
              <div style={{ padding: `${spacing.md} ${spacing.lg}`, borderBottom: `1px solid ${colors.border}` }}>
                ...
                {currentUser.email && (
                  <div style={{ fontSize: '12px', color: colors.textSecondary, marginTop: '2px' }}>{currentUser.email}</div>
                )}
              </div>
              ...
              {[
                ...
                <button key={label} onClick={action} style={{ width: '100%', padding: `${spacing.md} ${spacing.lg}`, background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: spacing.sm, color: colors.textPrimary, fontSize: '14px', transition: 'background 0.15s' }}
                  onMouseEnter={e => e.currentTarget.style.background = colors.background}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >
