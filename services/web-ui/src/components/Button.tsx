...
export default function Button({
  ...
  fullWidth = false, style = {}, type = 'button',
}: ButtonProps) {
  ...
  const variantStyles = {
    ...
    success:   { background: gradient.success, color: '#fff', border: 'none', shadow: '0 2px 8px rgba(16,185,129,0.3)', hoverShadow: '0 4px 16px rgba(16,185,129,0.4)' },
  };
  ...
  return (
    <button
      ...
      style={{
        ...
        fontWeight: '600', letterSpacing: '-0.1px', borderRadius: sz.radius,
        background: vr.background, color: vr.color, border: vr.border,
        boxShadow: pressed ? 'none' : vr.shadow,
        cursor: isDisabled ? 'not-allowed' : 'pointer',
        opacity: isDisabled ? 0.55 : 1, transition: 'all 0.15s ease',
...
function LoadingDot({ color }: { color: string }) {
  return (
    <span style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
      {[0, 1, 2].map(i => (
        <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', backgroundColor: color, animation: `btnDot 1.2s ease-in-out ${i * 0.2}s infinite`, opacity: 0.7 }} />
      ))}
      <style>{`@keyframes btnDot { 0%,80%,100%{transform:scale(0.6);opacity:0.4} 40%{transform:scale(1);opacity:1} }`}</style>
    </span>
  );
}
